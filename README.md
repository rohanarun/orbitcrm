...
Relationship context, pipeline evidence, and AI-assisted next actions in a single workspace.

OrbitCRM is a focused, public MIT distribution for the `crm` module in [managed-oss-cloud](https://github.com/rohanarun/managed-oss-cloud). It includes a product web UI, a product-scoped HTTP client, the `orbitcrm` CLI, and a stdio MCP server exposing only this product's 9 typed actions.

![OrbitCRM sample workspace](./docs/product-workspace.png)
...
npm install
npm link
export ORBITCRM_TOKEN="a-scoped-workspace-token"
export ORBITCRM_URL="https://cloud.getsupers.com"
orbitcrm actions
orbitcrm workspace
orbitcrm page '{"limit":50,"state":"active","search":"Title prefix"}'
orbitcrm detail '00000000-0000-4000-8000-000000000001'
orbitcrm action account-upsert '{"externalKey":"acct-42","name":"Northwind","domain":"northwind.example","idempotencyKey":"crm.account-upsert.sample-0001"}'
```
...
export ORBITCRM_TOKEN="a-scoped-workspace-token"
export ORBITCRM_URL="https://cloud.getsupers.com"
export ORBITCRM_WEB_KEY="a-separate-random-browser-key"
npm start
```
...
```json
{
  "mcpServers": {
    "orbitcrm": {
      "command": "orbitcrm-mcp",
      "env": {
        "ORBITCRM_TOKEN": "a-scoped-workspace-token",
        "ORBITCRM_URL": "https://cloud.getsupers.com"
      }
    }
  }
}
...
Then point `ORBITCRM_URL` at the self-hosted control-plane origin. All products may share the same backend and PostgreSQL cluster while the backend preserves workspace and module boundaries.

## Typed action catalogue

| Action ID | Capability | Token scope | Operation |
|---|---|---|---|
| `account-upsert` | Upsert organization account | `write` | `command` |
| `contact-link` | Link contact to account | `write` | `command` |
| `opportunity-open` | Open qualified opportunity | `write` | `command` |
| `opportunity-transition` | Transition opportunity stage | `write` | `command` |
| `activity-record` | Record customer activity | `write` | `command` |
| `duplicate-review-propose` | Propose duplicate review | `ai` | `ai` |
| `next-action-propose` | Propose next relationship action | `ai` | `ai` |
| `pipeline-forecast` | Compute transparent pipeline forecast | `read` | `read` |
| `crm-export` | Export CRM graph | `write` | `command` |
